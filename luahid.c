...
static const struct hid_device_id *luahid_setidtable(lua_State *L)
{
	struct hid_device_id *user_table = NULL;
...
/*
 * Probe function for the HID driver.
 * This function also serves the match part
 */
...
static int luahid_register(lua_State *L)
{
	...
	luaL_checktype(L, 1, LUA_TTABLE); /* id_table */
	if ((user_driver->id_table = luahid_setidtable(L)) == NULL)
